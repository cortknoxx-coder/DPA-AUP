...
void ledTick() {
  ...
  // ── NEW BASE PATTERNS ──────────────────────────────────
    ...
    int head2 = (int)(((phase + 0.5f) * NUM_LEDS)) % NUM_LEDS;  // 180° offset

    // Comet 1 (main color, forward)
    for (int t = 0; t <= 4; t++) {
    ...
    // Comet 2 (complementary color, opposite direction)
    CRGB color2 = color;
    // Shift hue by ~120° for contrast
    CHSV hsv = rgb2hsv_approximate(color2);
    hsv.hue += 85;  // ~120° shift
    color2 = hsv;
    for (int t = 0; t <= 4; t++) {
      int idx = (head2 + t) % NUM_LEDS;  // opposite direction
      float fade = (t == 0) ? 1.0f : powf(0.4f, (float)t);
      CRGB c = color2;
      c.nscale8((uint8_t)(fade * 255));
      leds[idx] |= c;
    ...
    // Meteor Rain: random meteors streak down the strip
    FastLED.setBrightness(bright);
    // Fade all LEDs slightly each frame (persistence)
    for (int i = 0; i < NUM_LEDS; i++) {
      leds[i].nscale8(180);  // ~70% persistence
    }
    // Random meteor heads
    static uint8_t meteorPos = 0;
    static uint8_t meteorSpeed = 0;
    static unsigned long lastMeteorMs = 0;

    unsigned long ms = millis();
    if (ms - lastMeteorMs > 40) {  // 25fps
      lastMeteorMs = ms;
      meteorPos += 1;
      if (meteorPos >= NUM_LEDS + 5) {
        meteorPos = 0;
        meteorSpeed = random8(1, 3);
      }
      int pos = meteorPos;
      if (pos < NUM_LEDS) {
        leds[pos] = color;
